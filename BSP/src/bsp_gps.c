...
void gps_init(void)   //gps初始化
{
	...
	int i;
	Clear_Buffer();
	u8 txbuffer1[]="AT$MYGPSSTATE\r\n"; //查询GPS开启情况
	tx_size = sizeof(txbuffer1);
	memcpy(txbuffer,txbuffer1,tx_size);
	usart_interrupt_enable(USART0, USART_INT_TBE);
	bsp_DelayMS(500);
	strx=strstr((const char*)rxbuffer,(const char*)"gps opened");//判断结果
	while(strx==NULL)
	{
		Clear_Buffer();
		u8 txbuffer2[]="AT$MYGPSPWR=1\r\n"; 				//开启GPS服务
	...
	Clear_Buffer();
	u8 txbuffer3[]="AT$MYGNSSSEL=0\r\n"; 				//打开北斗定位
	tx_size = sizeof(txbuffer3);
	memcpy(txbuffer,txbuffer3,tx_size);
	usart_interrupt_enable(USART0, USART_INT_TBE);
	bsp_DelayMS(300);
	strx=strstr((const char*)rxbuffer,(const char*)"OK");//判断结果
	
for(i=0;i<5;i++)
{
	Clear_Buffer();
	u8 txbuffer4[]="AT$MYGNSSMSG\r\n"; 				//获取 GPS 辅助定位信息
	tx_size = sizeof(txbuffer4);
	memcpy(txbuffer,txbuffer4,tx_size);
	usart_interrupt_enable(USART0, USART_INT_TBE);
	bsp_DelayMS(2000);
	strx=strstr((const char*)rxbuffer,(const char*)"OK");//判断结果
	GPS_Analysis(&gpsx,(u8*)rxbuffer);//分析字符串
	Gps_Msg_Show();				//显示信息	
}
