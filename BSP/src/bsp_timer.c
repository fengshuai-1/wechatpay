...
uint8_t t1_flag = 0 ;  //定时器1标志位
uint8_t t2_flag = 0;   //定时器2标志位
...
void TIMER1_IRQHandler(void)  //1s中断 查询支付接口
{
...
void TIMER2_IRQHandler(void)  //1s中断 计时收费
{
...
void timer2_config(void)  //60s
{
    ...
    timer_initpara.prescaler         = 11999;
    timer_initpara.alignedmode       = TIMER_COUNTER_EDGE;
    timer_initpara.counterdirection  = TIMER_COUNTER_UP;  //向上对齐模式
