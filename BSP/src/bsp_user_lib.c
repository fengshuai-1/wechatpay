...
uint16_t GetMaxVal(uint16_t* arr, uint16_t len)
{
    ...
    uint16_t maxVal = arr[0]; //假设最大为arr[0]
    
    for(uint16_t i = 1; i < len; i++)  //遍历比较，找到大的就赋值给maxVal
    {
        if(arr[i] > maxVal)
            maxVal = arr[i];
    }
...
//{
//    uint32_t tmpArrLen = GetMaxVal(arr , len) + 1;

//    uint16_t tmpArr[tmpArrLen];  //获得空桶大小
//    uint32_t i, j;
//    
