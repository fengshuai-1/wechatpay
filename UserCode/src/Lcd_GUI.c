 ...
 * @retvalue   :None
******************************************************************************/   	  
void LCD_ShowString(uint16_t x,uint16_t y,uint8_t size,uint8_t *p,uint8_t mode,uint16_t Point_Color)
{         
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {   
 ...
 * @parameters :x:the bebinning x coordinate of the number
                y:the bebinning y coordinate of the number
                                ...
                                IsZero: 1:显示高位的0    0：不显示高位的0
 * @retvalue   :None
******************************************************************************/  			 
void LCD_ShowNum(uint16_t x,uint16_t y,uint32_t num,uint8_t len,uint8_t size,uint16_t Point_COLOR, uint16_t Back_COLOR , uint8_t IsZero)
{         	
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
 ...
 * @retvalue   :None
******************************************************************************/ 
void Gui_Drawbmp16(uint16_t x,uint16_t y,const unsigned char *p,uint8_t lenth) //显示40*40 QQ图片
{
  	int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+lenth -1 ,y + lenth -1);//窗口设置
...
/*****************************************************************************
 * @name       :void Gui_DrawBat(uint16_t x,uint16_t y,const unsigned char *p)
 * @date       :2018-08-09 
 * @function   :Display a 16-bit BMP image
 * @parameters :x:the bebinning x coordinate of the BMP image
                y:the bebinning y coordinate of the BMP image
 ...
 * @retvalue   :None
******************************************************************************/ 
void Gui_DrawBat(uint16_t x,uint16_t y,const unsigned char *p) //显示40*40 QQ图片
{
  unsigned	int i; 
	unsigned char picH,picL; 

	LCD_SetWindows(x,y,x+160-1,y+50-1);//窗口设置
...
*********************************************************************************************************
*/
void LCD_DrawPoints(uint16_t *x, uint16_t *y, uint16_t _usSize)
{
	uint16_t i;

	for (i = 0 ; i < _usSize - 1; i++)
	{
        LCD_DrawLine(x[i],  y[i], x[i + 1], y[i + 1]);
	}
}
